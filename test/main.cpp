...
int main(int argc, char *argv[]) {
  ...
  case bladerf_format::BLADERF_FORMAT_SC16_Q11_META:
    tx.valid_samples_per_buffer -= 16;
    rx.valid_samples_per_buffer -= 16;
  case bladerf_format::BLADERF_FORMAT_SC16_Q11:
    bytes_per_sample = 4;
    break;

  case bladerf_format::BLADERF_FORMAT_SC16_Q11_PACKED_META:
    tx.valid_samples_per_buffer -= 32;
    rx.valid_samples_per_buffer -= 32;
  ...
  skip_buffers = start_delay * rate / rx.samples_per_buffer / rx.nof_streams;
...
void *tx_stream_cb(struct bladerf *dev, struct bladerf_stream *stream, struct bladerf_metadata *meta, void *samples,
  ...
  if (has_meta && tx_timestamps) {
    ...
    if (tx.ts != 0) {
      *ts_ptr = tx.ts;
      tx.ts += tx.valid_samples_per_buffer / tx.nof_streams;
    }
  }
