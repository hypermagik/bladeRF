...
static const struct option long_options[] = {
    ...
    {"rx-buffer", required_argument, NULL, 'R'},
    {"tx-buffer", required_argument, NULL, 'T'},
...
static void usage(const char *argv0) {
  ...
  printf("  -S, --rate <n>            Sampling rate, in MHz\n");
  printf("                              default: 61.44\n");
  printf("  -B, --bitmode <mode>      Sample size, in bits\n");
  printf("                              <16|16m|12 (default)|12m|8|8m>\n");
  printf("  -R, --rx-buffer <n>       Number of samples per Rx buffer\n");
  printf("                              default: 8192\n");
  printf("  -T, --tx-buffer <n>       Number of samples per Tx buffer\n");
  printf("                              default: 8192\n");
...
void parse_opts(int argc, char **argv) {
  ...
  int opt_ind = 0;
  while (opt != -1) {
    opt = getopt_long(argc, argv, "d:S:B:R:T:r:t:sv:V:h", long_options, &opt_ind);
    ...
    case 'B':
      ...
      break;
    case 'R':
      rx.samples_per_buffer = atoi(optarg);
      break;
    case 'T':
      tx.samples_per_buffer = atoi(optarg);
      break;
    case 'r':
